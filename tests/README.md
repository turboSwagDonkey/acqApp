...
| `test_pupil_limit` | The animal is head-fixed, so the eye lives in one part of the frame and everything else dark in IR — fur, orbit, headplate — is larger than the pupil. That is why auto-seeding never worked here: `coarse_seed` saw a dark mask over half the sensor and bailed at its own guard. The user-set limit circle bounds the seed and refuses a fit centred outside it. Checked against controls that fail without it: the unrestricted seed gives up on the rig's framing, and prefers a bigger decoy blob when it doesn't. Also that the crop done for speed doesn't shift the answer, that a drag writes back as **one** settings change (three would drop the annulus lock three times), and that a click outside the limit is refused rather than silently seeding a search that can only fail. Records the trap the limit inherits: draw it tight around the pupil and the >50 %-dark guard fires inside the circle instead. |
