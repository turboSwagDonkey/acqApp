...
Diagnostic prints use "→ ≤ ⚠ Δ ─", which cp1252 can't encode — and Python
...
preset maths doesn't silently rewrite someone's stdout.
...
def enable_safe_console() -> None:
    ...
    Streams that can't be reconfigured (pythonw has no stdout; notebooks and
