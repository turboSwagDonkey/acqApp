...
**EyeLoop pupil tracking is on master and complete in the app** (2026-08-26,
operator's call, superseding the 2026-08-24 retirement). `eyeloop_tracker.py`
is the only file that touches EyeLoop; the hand-rolled tracker **stays retired**
in `archive/pupil_tracking/` — do not restore *that* one. The `pupil-tracking`
branch is stale: its work landed here instead.

**EyeLoop is GPL-3.0 and none of it is vendored** — imported from a clone at
`../eyeloop`, which is in no repo, so a fresh machine needs `git clone` +
`git apply docs/eyeloop-3.14-patches.diff`, or `ACQAPP_EYELOOP_DIR`. Without a
clone the pupil camera works exactly as before (a tested contract, not a hope).
**This repo is public**, so vendoring would make acqApp a derived work — the
operator's call, still open. docs/EYELOOP-INTEGRATION.md.

**There are SIX pupil clips on `E:`, not one** (2026-08-22 — four sessions
measured the first and generalised from it). The only real footage here:
...
is the easy one every earlier number came from (median 49); `State` FOV1_T1 is
the **operator's** — median 37, a low-contrast almond whose pupil barely
separates from the iris. **Measure any pupil change on both.** No clip has a
blink.
...
Use the **absolute** path: the shell usually starts in `Desktop\python`, the
*parent* of this repo, where the relative one resolves to nothing and Python
reports a baffling "the module '.venv' could not be loaded". Python here is
**3.14**, and **cv2 does install on it** (opencv-python 5.0.0.93, a `cp37-abi3`
wheel, in `.venv` with PyYAML). `avi.py`'s hand-written RIFF reader is still
what reads the clips, but it is no longer forced.
...
**The trap to know before touching pupil tracking: fit rate is not accuracy.**
...
Steps 2–7 are built; **step 8 is a rig trip**, and the rest of the traps are in
docs/EYELOOP-INTEGRATION.md.

**Experiment routines are built and their panel runs itself** (2026-08-26).
`routines/` is the protocol + the engine, both **Qt-free and callable-driven**;
`adapters/routines.py` is the only part that touches a device, and **Start opens
the recording it needs** (`MainWindow.set_recording`, the twin of `set_live`).
...
**The camera numbers. Nothing in the path is the constraint any more.** Grab
runs at 92 % of the link (105.92 fps / 2223 MB/s full frame). The writer's
direct-chunk path (1304 → 2696 MB/s) and a 2 GB ring ended the 53 %-of-frames
loss, so **full-frame bin 1 records complete in the bench**; 2×2 binning is now
only a way to use less disk. **It has not run against the camera** —
`WRITER_MBPS = 1800` is a guessed derate, and §6 item 1 is the measurement.

**The disk was never the constraint**, though DECISIONS.md called it "already at
a hardware limit" for a week: D: writes 2700 MB/s from a plain file, and the
1004 was one line of Python. **Measure the floor before calling something
floor-bound.**
...
`turboSwagDonkey/acqApp` (private), branch `master`. **This box is the rig**, so
a commit here is already on the machine that runs it — push anyway, the remote
is the backup. `.gitignore` covers `sessions/`, `*.h5`, `*.csv`, `*_local.json`,
`.venv/`, so a commit cannot push experiment data. **Commit before
restructuring, push before leaving**: this once drifted six weeks and 68 files
with a 950-line refactor on top.

## 4. Stages

**Phases 0–5 done and mock-verified**; phase 0's numbers are in §0. Phase 6
(hardware sync — `DaqClock` on the PCIe-6363, triggered ORCA) is future work
and nothing depends on it. The per-phase table is in
...
Item numbers (#1–#20, C1–C3, B1–B2) are referenced from §5b, the session log and
...
- **Open/Closed is real.** Adding an instrument is a `ModuleAdapter` subclass
  plus a registry line; the only module names in `main.py` are two colour
  lookups. That is #17 having worked — defend it in review.
...
**A1, A2, A4 and A5 are closed** — declared `Protocol`s in `acq/devices.py`,
the three registrations stated at `ADAPTERS`, `ModuleHost` checked in both
directions, and `adapters/` replacing `modules.py` with `main.py` deliberately
left alone. Reasoning in [docs/DECISIONS.md](docs/DECISIONS.md); **A3 is the
one still open:**
...
**Everything below is the open tail, one line each.** The reasoning lives in
...
- **`per_step` save mode does not roll files.** Modelled, validated and carried
  ...
  `/routine` boundaries. Rolling means re-entering
...
- **Its first real run is a rig trip**, after item 2 above: a routine that
  ...
  one decision of six they did not state themselves (`routines/engine.py`).

**EyeLoop pupil tracking** — integrated and complete in the app, 2026-08-26.
Measurements in [docs/EYELOOP.md](docs/EYELOOP.md); how each step closed, the
traps table and what not to rebuild in
[docs/EYELOOP-INTEGRATION.md](docs/EYELOOP-INTEGRATION.md). **Read the traps
before changing any of it** — every one of them reports success while being
wrong.

- **Step 8, the live Basler**, is all that is left of the build. Not an
  actuation: the pupil camera only looks. Turn tracking on, place the eye
  region on the real eye, watch the ellipse, record 30 s.
- **The operator's tuned numbers are not written down.** They persist, so they
  are in `acqapp_local.json` on this machine only; which ones were settled on,
  and whether they should be the shipped defaults, is unrecorded.
- **Whether one threshold holds for a session** or tracks illumination. The
  single most consequential number and nothing measures it.
- **Ground truth.** Every number so far is a proxy;
  `archive/pupil_tracking/_mark_truth.py` exists to fix that and has never run.
- **GPL-3.0 is still the operator's call.** Nothing is vendored — a fresh
  ...
  Vendoring into `devices/` makes acqApp a derived work and **this repo is
  public**. `EyeLoopUnavailable` leaves the pupil camera exactly as it was (a
  tested contract), so the decision blocks nothing.

~~Pupil tracking (the hand-rolled one).~~ **Retired 2026-08-24** —
`archive/pupil_tracking/`, whose README has the measurements and the restore
steps. Do not restore without being asked; EyeLoop is the live thread.

**Older items** — closed ones, and the long-tail open ones (full-frame writer
throughput, closing the loop on the rig, which wheel speed a rule watches) — are
in [docs/DECISIONS.md](docs/DECISIONS.md) with their reasoning. Two there are
still worth doing, under "Still open".
...
§6 item 5's two jobs over this session's code. **First pass where the prose
half was the one that mattered** — only because the code swept was written the
same day: `track_worker.py` 52.5 → 44.9 %, `settings.py` 59.5 → 57.0 %, tree at
27.6 %. **The optimisation half found nothing to fix and one thing not to**: the
profile blamed the tracking wrapper for 0.16 ms/frame, and timed directly it is
2.042 vs 2.048 ms — zero. That was `sys.monitoring`'s own per-call cost, the
mirror of (at)'s trap where cProfile *under*-attributed. Also: a tick benchmark
must be **paced** (a tight loop reports 0.005 ms, timing a tick with no frame to
draw); paced, it is 4.14 ms with eight modules and tracking on. One dead name,
two unused imports. Numbers in DECISIONS §6 item 5.

Then, at the operator's request, **the per-session reading cost**: PLAN.md
12.7k → ~7k tokens by archiving four §7 entries, dropping four §5b items already
written up in DECISIONS, cutting an EyeLoop reference block that both
EYELOOP*.md files already hold, and deleting a §0 paragraph that had become
false (tracker work "happens on the branch, not here" — it landed here). §8 now
carries a **size budget**, since a soft rule is what let it double. With both
CLAUDE.md files trimmed the mandatory per-session read is **14.3k → 9.1k
tokens**. `Report` also grew **`-q`** (failures + count only: 134 lines → 3),
and `run_all` no longer dumps a failing test's passing lines.

### 2026-08-26 (ba) — the routines panel: one Start button, and real editors

Operator's request, both halves. Suite 1001 → 1044 checks.

- **Start opens its own recording** — it used to refuse until Record had been
  pressed elsewhere in the window. `MainWindow.set_recording` is the twin of
  `set_live` (§5b A4) and returns the PREVIOUS state, which is what lets the
  adapter stop a recording it started and leave the operator's running. The
  operator approved the main.py edit. `RigLimits.has_frames` changed meaning
  with it: "a camera is loaded", not "a file is open".
- **`routines/table.py`** — every cell edits through a widget that can only
  ...
  Plus reorder arrows, "No pattern", the running row in bold, and a summary line.
- **`style.toggle_btn` had no `:disabled` rule**, the wart `solid_btn`'s own
  docstring warns about — so Emulate and Free run, disabled for the whole of a
  session, still rendered as the thing to press. Fixed in style.py.
- **The suite's one flaky test was a cross-thread race in the test**, not in the
  ...
  so "one recorded event per fire" compared two counters mid-flight. Stop, pump,
  then count. There is now a check that the gaps are never shorter than the
...
1. Tick what actually got done — and only what was *verified*, with the test or
   the command that verified it. (§5b, or §5's one remaining item.)
2. Rewrite §6 "Next actions" (3 items max, ordered).
3. Add one dated entry to §7, newest first, **and move the oldest out in the
   same commit** if that makes four. Archiving later is what never happens.
...
**The size budget: ~450 lines, ~7k tokens.** Check it (`wc -l PLAN.md`) when
you edit this file — it is read in full at the start of every session, so every
line is paid for again on every future session, and a soft "aim to stay short"
is exactly what let it reach 818 lines by 2026-08-26. When it is over: archive
finished §7 entries, and cut anything a `docs/` file already says in full. What
belongs *here* is what a fresh session must know before it can act. It went 676 → 417 lines on 2026-08-12 by
